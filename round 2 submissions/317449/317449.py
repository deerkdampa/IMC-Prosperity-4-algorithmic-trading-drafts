    ...
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ...
                fair = 0.5 * 10000.0 + 0.5 * ema

                current = pos

                # Take only clearly mispriced levels.
                for ask, vol in sorted(depth.sell_orders.items()):
                    if ask <= fair - 2 and current < limit:
                        qty = min(-vol, limit - current)
                        if qty > 0:
                            orders.append(Order(product, ask, qty))
                            current += qty
                ...
                for bid, vol in sorted(depth.buy_orders.items(), reverse=True):
                    if bid >= fair + 2 and current > -limit:
                ...
                # === INVENTORY SHAVING LOGIC ===
                # load_factor goes from -1.0 to 1.0
                load_factor = current / limit
                
                # # 'shave_multiplier' determines how many ticks we push our price 
                # # to get rid of inventory. Setting this to 5 is quite aggressive.
                # shave_multiplier = 6
                # shave = int(round(load_factor * shave_multiplier))
                # The "Cubic Shave" ensures we don't get stuck at +80/-80
                shave = (load_factor ** 3) * 10.0
                # # Passive market making with modest inventory skew.
                # skew = int(round(current * 0.0482))
                half_spread = 1 if spread <= 4 else 2
