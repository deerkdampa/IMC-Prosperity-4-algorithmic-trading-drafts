    ...
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ...
                fair = 0.75 * fast + 0.25 * slow

                if slope > 2 or momentum > 1:
                    hold_mode = True
                if slope < -5 and momentum < -2:
                    hold_mode = False
                ...
                take_cap = fair if not hold_mode else fair + 1
                ...
                if current < desired:
                    ...
                    qty = min(desired - current, 16)
                    if qty > 0 and bid_px > 0:
                        orders.append(Order(product, int(bid_px), int(qty)))

                if current > 0 and (slope < -3 or momentum < -3):
                    rich_price = math.ceil(fair + 2)
                ...
                for ask, vol in sorted(depth.sell_orders.items()):
                    ...
                    if ask <= fair - 2:
                        ...
                        if qty > 0:
                            orders.append(Order(product, ask, qty))
                            current += qty
                    elif ask <= fair - 1 and spread >= 8 and current < 40:
                        qty = min(-vol, min(limit - current, 10))
                        if qty > 0:
                            orders.append(Order(product, ask, qty))
                            current += qty
                    else:
                ...
                for bid, vol in sorted(depth.buy_orders.items(), reverse=True):
                    ...
                    if bid >= fair + 2:
                        ...
                        if qty > 0:
                            orders.append(Order(product, bid, -qty))
                            current -= qty
                    elif bid >= fair + 1 and spread >= 8 and current > -40:
                        qty = min(vol, min(current + limit, 10))
                        if qty > 0:
                            orders.append(Order(product, bid, -qty))
                            current -= qty
                    else:
                ...
                skew = int(round(current * 0.03))
                half_spread = 2 if spread <= 7 else 3
                ...
                buy_size = min(12, buy_cap)
                sell_size = min(12, sell_cap)

                if current > 50:
                    buy_size = min(buy_size, 4)
                if current < -50:
                    sell_size = min(sell_size, 4)
