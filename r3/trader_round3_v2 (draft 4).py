...
class Trader:
    ...
    # Round 3 starts with 5 days left to expiry.
    T = 5.0 / 365.0
    ...
    def hydro_strategy(self, depth, pos):
        ...
        if len(self.hydro_mids) >= 6:
            anchor = 0.75 * 10000.0 + 0.25 * (sum(self.hydro_mids[-6:]) / 6.0)

        # modest mean reversion with inventory skew
        fair = anchor
        skew = pos / 40.0
        fair -= self.clamp(skew, -3.0, 3.0)

        # take only clear edge
        pos = self.take_buy("HYDROGEL_PACK", depth, fair, pos, orders, max_qty=12, edge=1.0)
        pos = self.take_sell("HYDROGEL_PACK", depth, fair, pos, orders, max_qty=12, edge=1.0)

        bid, ask = self.best_bid_ask(depth)
        if bid is not None and ask is not None:
            mm_bid = int(math.floor(fair - 4 - self.clamp(pos / 50.0, -2, 2)))
            mm_ask = int(math.ceil(fair + 4 - self.clamp(pos / 50.0, -2, 2)))
            mm_bid = min(mm_bid, bid + 1)
            mm_ask = max(mm_ask, ask - 1)
            if mm_ask <= mm_bid:
                mm_ask = mm_bid + 1
            buy_qty = min(8, self.can_buy("HYDROGEL_PACK", pos))
            sell_qty = min(8, self.can_sell("HYDROGEL_PACK", pos))
            if buy_qty > 0:
    ...
    def extract_strategy(self, depth, pos):
        ...
        fair = ema + self.clamp(1.3 * trend, -2.5, 2.5)
        fair += self.clamp(-pos / 60.0, -2.0, 2.0)

        pos = self.take_buy("VELVETFRUIT_EXTRACT", depth, fair, pos, orders, max_qty=16, edge=1.0)
        pos = self.take_sell("VELVETFRUIT_EXTRACT", depth, fair, pos, orders, max_qty=16, edge=1.0)

        bid, ask = self.best_bid_ask(depth)
        if bid is not None and ask is not None:
            mm_bid = int(math.floor(fair - 2 - self.clamp(pos / 70.0, -2, 2)))
            mm_ask = int(math.ceil(fair + 2 - self.clamp(pos / 70.0, -2, 2)))
            mm_bid = min(mm_bid, bid + 1)
            mm_ask = max(mm_ask, ask - 1)
            if mm_ask <= mm_bid:
                mm_ask = mm_bid + 1
            buy_qty = min(10, self.can_buy("VELVETFRUIT_EXTRACT", pos))
            sell_qty = min(10, self.can_sell("VELVETFRUIT_EXTRACT", pos))
            if buy_qty > 0:
    ...
    def voucher_fair(self, s, k, sigma):
        ...
        # Deep ITM should hug intrinsic; ATM and OTM get more optionality.
        moneyness = s - k
        if moneyness >= 700:
            return intrinsic
        if moneyness >= 300:
            return max(intrinsic, 0.88 * intrinsic + 0.12 * bs)
        if moneyness >= 100:
    ...
    def voucher_strategy(self, state, extract_mid):
        ...
        # trade only the strikes with the best signal-to-noise
        priority = ["VEV_5000", "VEV_5100", "VEV_5200", "VEV_5300", "VEV_5400"]
        secondary = ["VEV_4500", "VEV_5500"]
        ultra_illiquid = ["VEV_4000", "VEV_6000", "VEV_6500"]
        ...
        for sym, k in self.STRIKES.items():
            ...
            # Strike-specific vol tilt: higher strikes get slightly more vol.
            vol_tilt = 1.0
            ...
            # Deep ITM vouchers: only trade if the market strays from intrinsic.
            if sym in ultra_illiquid:
                ...
                if sym in ("VEV_6000", "VEV_6500"):
                    # skip unless absurdly mispriced
                    if mid > 1.5:
                        fair = 0.0
                    else:
                        result[sym] = []
            ...
            mis = fair - mid
            spread = max(1.0, self.best_bid_ask(depth)[1] - self.best_bid_ask(depth)[0]) if self.best_bid_ask(depth)[0] is not None and self.best_bid_ask(depth)[1] is not None else 2.0
            confidence = self.clamp(abs(mis) / (spread + 1.0), 0.0, 2.0) / 2.0

            # Cap how much of the limit we are willing to use.
            if sym in priority:
                size_cap = 32
                scale = 0.22
            ...
            # Clear mispricing threshold to avoid overtrading.
            if abs(mis) >= max(1.5, 0.75 * spread):
                size = int(min(size_cap, self.LIMITS[sym] * scale * (0.35 + confidence)))
                if abs(mis) >= 4.0:
                    size = min(size_cap, max(size, 18 if sym in priority else 10))
            ...
            # Passive quoting only where there is reasonable liquidity.
            if sym in priority + secondary:
                bid, ask = self.best_bid_ask(depth)
                if bid is not None and ask is not None:
                    skew = self.clamp(pos / 120.0, -2.5, 2.5)
                    ...
                    quote_qty = 4 if sym in secondary else 6
                    if sym == "VEV_5300":
                        quote_qty = 8
    ...
    def run(self, state: TradingState):
        try:
            ...
            # Extract last, used to hedge voucher delta but keep some directional flexibility.
            if ext_depth is not None and extract_mid is not None:
                ...
                # Hedge a large fraction of the voucher delta, but not 100%.
                target = int(self.clamp(-0.70 * hedge_delta, -200, 200))
                gap = target - pos_e

                # If we need to move more than the fair-value logic would naturally do,
                # add a small hedge-only crossing component.
                if gap > 10:
