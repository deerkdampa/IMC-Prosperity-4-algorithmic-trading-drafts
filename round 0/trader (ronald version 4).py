        ...
        for product in state.order_depths:
            ...
            if product == "EMERALDS":
                ...
                if best_ask - best_bid <= 1:
                    ...
                    mm_buy_price = int(fair_value) - EMERALD_MM_SPREAD
                    mm_sell_price = int(fair_value) + EMERALD_MM_SPREAD
                else:
                    # If the book has a wider spread, quote one tick off fair to
                    # remain competitive while still collecting a spread.
                    mm_buy_price = min(int(fair_value) - 1, best_ask - 1)
                    mm_sell_price = max(int(fair_value) + 1, best_bid + 1)
            ...
            # -------------------------------------------------------
                ...
                buy_price  = min(round(fair_value) - spread, best_ask - 1)
                sell_price = max(round(fair_value) + spread, best_bid + 1)
